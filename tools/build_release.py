...
EXPECTED_SIZE = 95632
EXPECTED_LINES = 2865
if VERSION != "26.3.6": raise SystemExit("Unexpected VERSION")
...
out = ROOT / "releases" / "V26.3.6"
out.mkdir(parents=True, exist_ok=True)
timestamp = (2026, 9, 9, 0, 0, 0)
...
sd = out / "GIB2A-Xicoy-ProHub-Widget-V26.3.6-SD.zip"
make_zip(sd, [
    ("SCRIPTS/GIB2A/main.lua", raw),
    ("SCRIPTS/GIB2A/gib2a_logo_ethos_180.png", logo_raw),
])
suite = out / "GIB2A-Xicoy-ProHub-Widget-V26.3.6-ETHOS-Suite.zip"
...
(checksums / "GIB2A-V26.3.6.sha256").write_text(
    "\n".join(lines + [f"{EXPECTED_HASH}  src/GIB2A/main.lua"]) + "\n",
    encoding="ascii", newline="\n",
)
(out / "GITHUB_RELEASE_NOTES.md").write_bytes(
    (ROOT / "docs/release-notes/V26.3.6.md").read_bytes()
)
attachment_lines = "\n".join(f"- {path.name}" for path in sorted(archives))
handoff = f"""GIB2A V26.3.6 — GitHub Desktop handoff
...
Release GIB2A V26.3.6

Recommended commit description:
Prepare the GIB2A V26.3.6 public release with the validated Lua source, external logo, updated documentation, release notes and reproducible SD and ETHOS Suite packages.
...
Future release title: GIB2A V26.3.6
Future tag: v26.3.6

Repository description:
Multi-ECU turbine telemetry widget for FrSky ETHOS. Supports Xicoy, JetCat, EnJet, KingTech, Swiwin and Linton turbine systems.

Topics:
frsky, ethos, lua, telemetry, rc-jet, turbine, xicoy, jetcat, kingtech, swiwin, enjet, linton

RADIO VALIDATION: PASS.
Publication remains subject to explicit authorization.

Future steps only after explicit publication authorization:
