...
EXPECTED_SIZE = 86894
EXPECTED_LINES = 2622
if VERSION != "26.3.2": raise SystemExit("Unexpected VERSION")
...
sd = out / "GIB2A-Xicoy-ProHub-Widget-V26.3.2-SD.zip"
make_zip(sd, [
...
if manifest.exists():
    suite = out / "GIB2A-Xicoy-ProHub-Widget-V26.3.2-ETHOS-Suite.zip"
    make_zip(suite, [
...
(out / "GITHUB_RELEASE_NOTES.md").write_bytes(
    (ROOT / "docs/release-notes/V26.3.2.md").read_bytes()
)
attachment_lines = "\n".join(f"- {path.name}" for path in sorted(archives))
handoff = f"""GIB2A V26.3.2 — GitHub Desktop handoff
...
Release GIB2A V26.3.2

Recommended commit description:
Prepare the GIB2A V26.3.2 public release with the validated Lua source, external logo, updated documentation, release notes and reproducible SD and ETHOS Suite packages.
...
Future release title: GIB2A V26.3.2
Future tag: v26.3.2
