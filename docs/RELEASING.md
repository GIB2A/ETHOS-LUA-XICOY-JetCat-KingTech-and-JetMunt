...
git diff --check
git add .
git commit -m "Release GIB2A V26.3.6"
git push origin main
git tag -a v26.3.6 -m "GIB2A V26.3.6"
git push origin v26.3.6
```

The build generates exactly two official ZIPs (SD and ETHOS Suite) plus
`SHA256SUMS.txt`. After review, create a GitHub Release titled `GIB2A V26.3.6`,
paste the prepared release notes, and attach both validated ZIPs plus
`SHA256SUMS.txt`. Do not run these publication
